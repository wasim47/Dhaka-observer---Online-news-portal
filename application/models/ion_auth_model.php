...
//#############################################################################################################	 
	...
	public function createUser($additional_data){
	...
	/*public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		elseif ($this->identity_column == 'email' && $this->username_check($email))
		{
			$this->set_error('account_creation_duplicate_email');
			return FALSE;
		}
...
public function register($username, $password, $email, $additional_data, $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		$cols=$additional_data['role'];
			...
			$user_data = array_merge($this->_filter_data($this->tables['users'], $save_data), $data);
			$this->trigger_events('extra_set');
			$this->db->insert($this->tables['users'], $user_data);
		 }
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this
				->db
				->select($this->identity_column . ', username, email, id, user_id, first_name, last_name, password, active, role, last_login')
	...
	**/
	...
	function get_category_approve($approve_val)
	{
	   $setval = array(
		...
		$this->db->where('id', $approve_val);
		$this->db->update('users', $setval);
		return false;
	}
	...
	function get_category_deapprove($deapprove_val)
	{
		...
		$this->db->where('id', $deapprove_val);
		$this->db->update('users', $setval);
		return false;
	}
