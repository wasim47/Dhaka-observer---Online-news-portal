       ...
       $query = $this->News_model->get_all_news();
		...
		$str .='<rss version="2.0" xmlns:media="http://search.yahoo.com/mrss/" xmlns:atom="http://www.w3.org/2005/Atom">';
		  $str .='<channel>';
			...
			$str .='<atom:link href="'.base_url().'english/application/views/frontend/feeds/'.$sector_heading.'.xml" rel="self" type="application/rss+xml"/>';
...
$str .='<description>'.'<![CDATA['.stripslashes($sbuzz_description).']]'.'></description>'; 	
			 ...
			 $str .='</channel>';
			$str .='</rss>';
				if(!write_file('./english/application/views/frontend/feeds/'.$sector_heading.'.xml', $str))
				{
				}
				else
				{
